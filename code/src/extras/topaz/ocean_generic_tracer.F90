...
!!hcjung 20171010 add rad=par
    ...
    real, intent(in)                                                :: dtts
    real, optional, intent(in), dimension(Disd:,Djsd:,:)            :: rad
    type(ocean_prog_tracer_type), dimension(:), intent(inout)       :: T_prog
    ...
    call generic_tracer_source(T_prog(indtemp)%field(:,:,:,Time%taup1),&
         ...
         nbands, max_wavelength_band, sw_pen_band, opacity_band, Velocity%current_wave_stress(:,:), rad)
