...
!-----------------------------------------------------------------------
  ...
  !hcjung 20170630
  public user_allocate_arrays
  public user_deallocate_arrays
  !hcjung 20170630
...
!## jgj NOTE: to match mom4p1/gold
  ...
  subroutine user_add_tracers(tracer_list)
    ...
    call g_tracer_add_param('ocean_restart_file' , topaz%ocean_restart_file , 'ocean_topaz_column.res.nc' ) !!hcjung
...
!         prog       = .true.                          )
  ...
  subroutine generic_TOPAZ_update_from_source(tracer_list,Temp,Salt,rho_dzt,dzt,hblt_depth,&
    ...
    real, dimension(ilb:,jlb:,:),   intent(in) :: rad
...
!hcjung nbands=1
       ...
       do k = 1, nk !{
          ...
          topaz%irr_inst(i,j,k) = rad(i,j,k) * grid_tmask(i,j,k)
          topaz%irr_mix(i,j,k) = rad(i,j,k) * grid_tmask(i,j,k)
          !hcjung 20170812
...
!  subroutine generic_TOPAZ_set_boundary_values(tracer_list,SST,SSS,rho,ilb,jlb,tau) !hcjung 20180316
   subroutine generic_TOPAZ_set_boundary_values(tracer_list,SST,SSS,rho,ilb,jlb,tau,hist_o2_) !hcjung 20180316
    type(g_tracer_type),          pointer    :: tracer_list
    ...
    real, intent(in)  :: hist_o2_    !hcjung 20180316
    real, dimension(:,:,:)  ,pointer  :: grid_tmask
...
!       o2_csurf(i,j) = o2_field(i,j,1,tau) * topaz%Rho_0 !nnz: MOM has rho(i,j,1,tau) !hcjung 20180316
       o2_csurf(i,j) = hist_o2_ * topaz%Rho_0  !!hcjung 20180316
