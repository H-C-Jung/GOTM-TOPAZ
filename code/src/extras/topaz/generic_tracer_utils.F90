...
!-----------------------------------------------------------------------
  ...
  public :: g_tracer_set_common4column !hcjung 20170919
...
!hcjung 20170628
    !if(.NOT. _ALLOCATED(g_tracer_com%grid_tmask)) allocate(g_tracer_com%grid_tmask(isd:ied,jsd:jed,nk)) !hcjung 20170628
    if(_ALLOCATED(g_tracer_com%grid_tmask)) deallocate(g_tracer_com%grid_tmask)
    allocate(g_tracer_com%grid_tmask(isd:ied,jsd:jed,nk))
    g_tracer_com%grid_tmask=grid_tmask 


    !if(.NOT. _ALLOCATED(g_tracer_com%grid_kmt)) allocate(g_tracer_com%grid_kmt(isd:ied,jsd:jed)) !hcjung 20170628
    if(_ALLOCATED(g_tracer_com%grid_kmt)) deallocate(g_tracer_com%grid_kmt)
    allocate(g_tracer_com%grid_kmt(isd:ied,jsd:jed))
    g_tracer_com%grid_kmt = grid_kmt

    !if(.NOT. _ALLOCATED(g_tracer_com%grid_mask_coast)) allocate(g_tracer_com%grid_mask_coast(isd:ied,jsd:jed)) !hcjung 20170628
    if(_ALLOCATED(g_tracer_com%grid_mask_coast)) deallocate(g_tracer_com%grid_mask_coast)
    allocate(g_tracer_com%grid_mask_coast(isd:ied,jsd:jed))
!hcjung 20170628
...
!hcjung 20170919
  subroutine g_tracer_set_common4column(isc,iec,jsc,jec,isd,ied,jsd,jed,nk,ntau,axes,grid_tmask, mask_coast, grid_kmt, init_time)

    integer,                     intent(in) ::  isc,iec,jsc,jec,isd,ied,jsd,jed,nk,ntau,axes(3)
    real, dimension(isd:,jsd:,:),intent(in) :: grid_tmask
    integer,dimension(isd:,jsd:),intent(in) :: grid_kmt
    integer,dimension(isd:,jsd:),intent(in) :: mask_coast
    type(time_type),             intent(in) :: init_time
    integer :: i,j
    !Here we assume that all the tracers in the list have
    !the same following properties
    g_tracer_com%isd=isd
    g_tracer_com%ied=ied
    g_tracer_com%jsd=jsd
    g_tracer_com%jed=jed
    ...
    if(_ALLOCATED(g_tracer_com%grid_mask_coast)) deallocate(g_tracer_com%grid_mask_coast)
    allocate(g_tracer_com%grid_mask_coast(isd:ied,jsd:jed))
    g_tracer_com%grid_mask_coast = mask_coast 

  end subroutine g_tracer_set_common4column
!hcjung 20170919
